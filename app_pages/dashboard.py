"""Dashboard page: enter tickers -> score them (Factors 1 & 3 computed from
financials, Factors 2/4/5 drafted by Claude) -> check the boxes for
candidates worth a closer look -> generate a full playbook/report for
just those."""
...
num_tickers = st.number_input("How many tickers?", min_value=1, max_value=25, value=1, step=1)

ticker_rows = []
for i in range(int(num_tickers)):
    row_cols = st.columns([1, 2])
    with row_cols[0]:
        ticker_val = st.text_input(f"Ticker {i + 1}", key=f"ticker_{i}", placeholder="e.g. CP")
    with row_cols[1]:
        notes_val = st.text_area(
            f"Additional information {i + 1} (e.g. recent news, CEO tenure, ownership structure, "
            "suggested peer competitors)",
            key=f"ticker_notes_{i}",
            placeholder="e.g. NSC had a derailment/CEO scrutiny in 2023-2024; suggested peers: CP, CNI, UNP...",
            height=80,
        )
    ticker_rows.append((ticker_val, notes_val))

tickers = []
research_notes_by_ticker = {}
for ticker_val, notes_val in ticker_rows:
    ticker = ticker_val.strip().upper()
    if not ticker:
        continue
    if ticker not in research_notes_by_ticker:
        tickers.append(ticker)
    research_notes_by_ticker[ticker] = notes_val
        ...
        for i, ticker in enumerate(tickers):
            ...
            try:
                scores[ticker] = score_company(
                    ...
                    research_notes=research_notes_by_ticker.get(ticker, ""),
